...
{
    public partial class FliudBoomQuery : Form
    {
        #region AccessDll
        ...
        public FliudBoomQuery(Form1 Handle)
        {
            InitializeComponent();
        ...
        private string m_FilePath = Application.StartupPath + @"\FluidBoom\FluidBoobDataBase.mdb";//获取INI文件路径
        private string m_TableName = "TestResult";
        private string m_PicFilePath = Application.StartupPath + @"\FluidBoom\pressure.png";//获取INI文件路径
        private string m_ExcelModuleFilePath = Application.StartupPath + @"\FluidBoom\Report\FluidBoobModule.xls";//获取INI文件路径
        private string m_ExcelReportFilePath = Application.StartupPath + @"\FluidBoom\Report\";//获取INI文件路径
        private string m_INIstrFilePath = Application.StartupPath + @"\FluidBoom\info.ini";//获取INI文件路径
        ...
        private void FliudBoomQuery_Load(object sender, EventArgs e)
        {
            
            //m_AccessHandle.Init(m_FilePath, m_TableName, 1);
            ...
            m_MainFormHandle.m_FluidBoobAccessHandle.QueryData(com, DG_DataBase);
        ...
        {
            int Index = 0;
            m_MainFormHandle.m_FluidBoobAccessHandle.GetIndexBaseKeyWord(m_TableName, m_RecordId, ref Index);
            object data = default(object);
            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Product", ref data);
            string sProduct = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "TestNo", ref data);
            string TestNo = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "InspectDate", ref data);
            string sInspectDate = data.ToString();

            //m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "SetPressure", ref data);
            //float fSetPressure = Convert.ToSingle(data);

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "BurstPressure", ref data);
            float fBurstPressure = Convert.ToSingle(data);

            //m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "PressureRate", ref data);
            //float fPressureRate = Convert.ToSingle(data);

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Standard", ref data);
            string sStandard = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "EquipModule", ref data);
            string sModule = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "PrintDate", ref data);
            string sPrintDate = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Result", ref data);
            string sResult = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Tester", ref data);
            string sTester = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Reviewer", ref data);
            string sReviewer = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Admin", ref data);
            string sAdmin = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "Mark", ref data);
            string sMark = data.ToString();

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "StartTime", ref data);
            m_StartTime = Convert.ToDateTime(data);

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "yMax", ref data);
            m_yMax = Convert.ToInt32(data);

            float[] x;
            m_MainFormHandle.m_FluidBoobAccessHandle.ReadFloatsData(m_TableName, Index, "Point_x", out x);

            float[] y;
            m_MainFormHandle.m_FluidBoobAccessHandle.ReadFloatsData(m_TableName, Index, "Point_y", out y);

            m_MainFormHandle.m_FluidBoobAccessHandle.ReadSigleData(m_TableName, Index, "TestDataNum", ref data);
            ...
            float[] TestData;
            m_MainFormHandle.m_FluidBoobAccessHandle.ReadFloatsData(m_TableName, Index, "TestData", out TestData);
        ...
        private void SaveDataToDataBase()
        {
            ...
            int Index = 0;
            m_MainFormHandle.m_FluidBoobAccessHandle.GetIndexBaseKeyWord(m_TableName, m_RecordId, ref Index);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Product", sProduct);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "TestNo", TestNo);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "InspectDate", sInspectDate);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Standard", sStandard);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "EquipModule", sModule);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "PrintDate", sPrintDate);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Result", sResult);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Tester", sTester);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Reviewer", sReviewer);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Admin", sAdmin);
            m_MainFormHandle.m_FluidBoobAccessHandle.WriteSigleDataToExistCells(m_TableName, Index, "Mark", sMark);
